...
def update_dataset_table(selected_station: str, selection_store: Dict, selected_columns=None, domain_filters=None, stencil_filters=None, pml_filters=None):
    ...
    col_map = {
        ...
        "response": html.Th(
            html.Div([
                ...
                html.Div([
                    html.Span("E", style={"paddingRight": "12px"}),
                    html.Span("Q4", style={"paddingRight": "12px"}),
                    html.Span("Q8"),
    ...
    for base in base_order:
        ...
        q_options = [
            {"label": "", "value": "Q4", "disabled": not has_q4},
            {"label": "", "value": "Q8", "disabled": not has_q8},
        ]
        ...
        q_value = [q for q in q_value_raw if (q == 'Q4' and has_q4) or (q == 'Q8' and has_q8)]
                ...
                for qsel in (anel_sel if isinstance(anel_sel, list) else [anel_sel]):
                    ...
                    for p in p_list:
                        try:
                            ...
                            ds = p
                        if re.search(rf"anelastic[-_]?{re.escape(qsel)}", ds, flags=re.IGNORECASE):
                            found = p
                            break
